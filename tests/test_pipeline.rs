...
fn test_full_pipeline_reduces_features() {
    ...
    let missing_ratios = analyze_missing_values(&df).unwrap();
...
fn test_pipeline_handles_all_numeric_dataset() {
    ...
    let missing = analyze_missing_values(&df).unwrap();
    assert!(
...
fn test_csv_and_parquet_produce_same_results() {
    ...
    let missing_csv = analyze_missing_values(&df_csv).unwrap();
    let missing_parquet = analyze_missing_values(&df_parquet).unwrap();
