...
fn test_get_features_above_threshold() {
    ...
    assert!(
        ...
        "Should drop feature_b (35% missing)"
    );
    assert!(
        to_drop.contains(&"feature_c".to_string()),
        "Should drop feature_c (90% missing)"
    );
    assert!(
        !to_drop.contains(&"target".to_string()),
        "Should NOT drop target column"
    );
    assert!(
        !to_drop.contains(&"feature_a".to_string()),
        "Should NOT drop feature_a (10% missing)"
    );
}
...
fn test_get_features_threshold_boundary() {
    ...
    assert!(
        !to_drop.contains(&"exactly_at_threshold".to_string()),
...
fn test_empty_dataframe() {
    ...
    assert!(
        ratios.is_empty(),
        "Empty DataFrame should produce empty ratios"
    );
}
...
fn test_weighted_missing_ratio_vs_unweighted() {
    ...
    let ratio_uniform = ratios_uniform
        .iter()
        .find(|(n, _)| n == "feature")
        .unwrap()
        .1;
    ...
    let ratio_weighted = ratios_weighted
        .iter()
        .find(|(n, _)| n == "feature")
        .unwrap()
        .1;
