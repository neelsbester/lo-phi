...
fn test_cli_default_values() {
    ...
    assert_eq!(cli.missing_threshold, 0.3, "Default missing threshold should be 0.3");
    assert_eq!(cli.correlation_threshold, 0.95, "Default correlation threshold should be 0.95");
    assert_eq!(cli.gini_threshold, 0.05, "Default gini threshold should be 0.05");
    assert_eq!(cli.gini_bins, 10, "Default gini bins should be 10");
    assert!(!cli.no_confirm, "Default no_confirm should be false");
    assert_eq!(cli.infer_schema_length, 10000, "Default schema inference should be 10000");
}
...
fn test_cli_drop_columns() {
    let cli = Cli::parse_from([
        ...
        "-i", "data.csv",
        "-t", "target",
        "--drop-columns", "id,timestamp,uuid",
    ]);
